...
export default function billPage() {
    ...
    const { searchTerm, setSearchTerm } = useSearch(initialData, setDataBill, ['referensi_rfq', 'referensi_bill',]);

    const fetchData = async () => {
        try {
            const [responseBill] = await Promise.all([
                apiClient.get('/bill', { cache: 'force-cache' }),
            ]);

            setDataBill(responseBill.data.data);
            setInitialData(responseBill.data.data);
            setIsLoading(false);
        } catch (error) {
