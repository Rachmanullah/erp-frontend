...
export default function CustomerPage() {
    ...
    const { searchTerm, setSearchTerm } = useSearch(initialData, setDataCustomer, ['nama_customer', 'type', 'company_registrasi', 'alamat', 'no_telp', 'email']);
    const modalRef = useRef(null);

    const fetchDataCustomer = async () => {
        try {
            await apiClient.get('/customer', { cache: 'force-cache' })
                .then((res) => {
    ...
    const handleEdit = (customer) => {
        ...
        setFormData({
            ...
            company_registrasi: customer.company_registrasi,
            type: customer.type,
    ...
    const handleUpdate = async (e) => {
        ...
        setIsLoading(true);
        try {
            const response = await apiClient.put(`/customer/${selectedCustomer.id}`, formData);
            setIsLoading(false);
            Swal.fire({
                title: 'Success!',
                text: response.data.message,
                    ...
                    >
                        ...
                        <form className="space-y-6">
                            <div className="grid grid-cols-1 gap-4">
                                ...
                                <div>
                                    <label htmlFor="nama_customer" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Nama Customer</label>
                                    <input
                                        type="text"
                                        id="nama_customer"
                                        name="nama_customer"
                                    ...
                                    />
                                    {error?.nama_customer && <p className="text-red-500 text-sm mt-2">{error?.nama_customer}</p>}
                                </div>
                                <div>
