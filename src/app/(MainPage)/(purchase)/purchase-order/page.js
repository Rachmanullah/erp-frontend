...
export default function PurchaseOrderPage() {
    ...
    const { searchTerm, setSearchTerm } = useSearch(initialData, setDataPO, ['nama_vendor', 'total_pembayaran', 'referensi_rfq', 'confirmation_date', 'arrival_date', 'status']);

    const fetchData = async () => {
        try {
            await apiClient.get('/purchaseOrder', { cache: 'force-cache' })
                .then((res) => {
                                ...
                                <tr key={index} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex gap-2">
                                            <Link href={ROUTES.detailpurchaseOrder(item.id)} className="font-medium text-blue-600 dark:text-blue-500 hover:underline">Detail</Link>
                                        </div>
                                    </td>
                                </tr>
                            ))
